...
현재 검토 페이지는 구축 계획과 A/B/C/D 시안을 묶은 통합 리뷰 페이지다. 영상 기반 시안 A의 최신 키프레임·카메라 검토 기록은 [V2 기록](continuity-v2/README.md)을 참고한다. 현재 버전 검사는 `node output/design-preview-site/continuity-v2/qa.cjs`로 실행한다.
...
브라우저에서 통합 리뷰 페이지는 `http://127.0.0.1:8761/output/design-preview-site/`, 한국어 진입점은 `http://127.0.0.1:8761/output/design-preview-site/ko.html`을 연다. 구축 계획은 통합 페이지의 `00`, 영상 기반 시안은 `A` 탭에서 확인한다. HTML을 file URL로 직접 열면 일부 로컬 리소스 fetch가 제한될 수 있다.
...
```powershell
python output/design-preview-site/scripts/composite.py
python output/design-preview-site/scripts/contact-sheet.py
python output/design-preview-site/scripts/validate.py
node output/design-preview-site/scripts/browser-qa.cjs
python output/design-preview-site/scripts/final-report.py
```
